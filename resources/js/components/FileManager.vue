...
export default {
    ...
    methods: {
        ...
        async onDrop(event) {
            ...
            for (let i = 0; i < files.length; i++) {
                ...
                try {
                    ...
                    this.fullScreenLoader = false;
                    console.log('pooling thumbnail state');
                    this.pollThumbnailStatus(response.data.id, response.data);
                } catch (error) {
        ...
        pollThumbnailStatus(id, data) {
            setInterval(() => {
                // Make an AJAX request to the server to check the status
                axios.get(`/api/check-file-thumbnail-status/${id}`)
                    .then(response => {
                        let file = this.files.find((file) => file.id == response.data.id);
                        file.thumbnail = response.data.thumbnail;
                        clearInterval(this.pollThumbnailStatus);
                    })
                    .catch(error => {
                        console.error(error);
                    });
